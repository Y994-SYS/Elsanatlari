...
namespace Elsanatlari
{
	public partial class Form1 : Form
	{
		...
		{
			// Yalnızca MevcutStok sütununu kontrol et
			if (dataGridView1.Columns[e.ColumnIndex].Name == "MevcutStok" && e.RowIndex >= 0)
			{
				// Değerin null veya DBNull olmadığından emin ol
				if (e.Value != null && e.Value != DBNull.Value)
				{
					int stokDegeri = Convert.ToInt32(e.Value);

					if (stokDegeri < 0)
					{
						// Negatif Stok: Kritik Kırmızı
						e.CellStyle.BackColor = System.Drawing.Color.Red;
						e.CellStyle.ForeColor = System.Drawing.Color.White;
					}
					else if (stokDegeri <= 10)
					{
						// Düşük Stok: Uyarı Turuncusu/Sarı
						e.CellStyle.BackColor = System.Drawing.Color.Orange;
						e.CellStyle.ForeColor = System.Drawing.Color.Black;
					}
					else
					{
						// Normal Stok: Varsayılan (Temizleme gerekli)
