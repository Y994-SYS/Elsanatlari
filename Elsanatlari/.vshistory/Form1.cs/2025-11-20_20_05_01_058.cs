...
namespace Elsanatlari
{
	public partial class Form1 : Form
	{
		...
		{
			// *** ÜRÜN İŞLEMLERİ SIRALAMA FİLTRESİ (dataGridView1) ***
			// Not: Bu combobox'ın adı CmbFiltre/CmbSiralama olarak varsayılmıştır, 
			// ancak kodunuzda CmbUrunFiltre olarak kullanıldığı için o isimle devam edilmiştir.

			// UrunleriListele metodu CmbUrunFiltre'yi (Stok Hareketleri Ürün Filtresi olarak) sıfırlayıp doldurduğu için, 
			// karışıklığı önlemek adına Ürün İşlemleri Sıralama Filtresinin adının Form tasarımında CmbFiltre olduğunu varsayalım.
			// Eğer arayüzde CmbFiltre diye bir kontrol yoksa ve bunun yerine CmbUrunFiltre kullanılıyorsa, lütfen bunu CmbUrunFiltre yapınız.

			// BURAYI CmbFiltre olarak varsayıyorum.
			...
			// *** STOK HAREKETLERİ FİLTRESİ (Hareket Türü) (dataGridView2) ***
		...
		private void UrunleriListele()
		{
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// Stok Hareketleri Ürün Filtresi (CmbUrunFiltre)
				CmbUrunFiltre.Items.Clear();
		...
		private void HareketleriListele()
		{
			...
			{
				// Eğer hiçbir şey seçili değilse, varsayılan olarak ilk öğeyi ("Tümü") seç
				cmbHareketFiltre.SelectedIndex = 0;
			}

			// Aynı kontrolü Ürün Filtresi için de yapalım
			if (CmbUrunFiltre.SelectedItem == null)
			{
				// "Tüm Ürünler" seçeneği (Value=0) varsayılan olmalı
				CmbUrunFiltre.SelectedIndex = 0;
			}
		...
		private void UrunFiltrele()
		{
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// CmbFiltre olarak varsayılmıştır. Eğer arayüzde adı CmbUrunFiltre ise burayı CmbUrunFiltre yapınız.
