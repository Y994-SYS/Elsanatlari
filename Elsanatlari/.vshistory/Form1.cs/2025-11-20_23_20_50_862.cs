	...
	public partial class Form1 : Form
	{
		...
		private void HareketleriListele()
		{
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// Sqlite için formatı kontrol edin.
				...
				// *** DÜZELTME: Sorguyu çalıştırıp tabloya bağlıyoruz. ***
				...
				// ** İkinci DataGridView'ınızın adı muhtemelen dataGridView2'dir. **
		...
		private void BtnGuncelle_Click(object sender, EventArgs e)
		{
			...
			// *** DÜZELTME: Mevcut Stok kontrolü ve alma eklendi ***
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				var cmd = connection.CreateCommand();
				// *** DÜZELTME: Sorguya MevcutStok alanı eklendi. ***
				cmd.CommandText =
				@"UPDATE Urunler SET
					UrunAdi = $a,
					StokKodu = $s,
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0)
			{
				// *** DÜZELTME: Tıklanan satırı seçili hale getiriyoruz. ***
		...
		private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0)
			{
				// *** DÜZELTME: Tıklanan satırı seçili hale getiriyorusz. ***
				...
				TxtHareketId.Text = row.Cells["Id"].Value.ToString();
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				try
				{
					...
					if (hareketTuru == "GİRİŞ")
					{
						cmdSil.CommandText = "UPDATE Urunler SET MevcutStok = MevcutStok - $m WHERE Id = $urunId;";
					}
					else
					{
						cmdSil.CommandText = "UPDATE Urunler SET MevcutStok = MevcutStok + $m WHERE Id = $urunId;";
					}
					cmdSil.Parameters.AddWithValue("$m", hareketMiktari);
					cmdSil.Parameters.AddWithValue("$urunId", urunId);
					cmdSil.ExecuteNonQueryAsync();
					trans.Commit();
