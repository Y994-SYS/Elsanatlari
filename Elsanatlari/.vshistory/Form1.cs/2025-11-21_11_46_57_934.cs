	...
	public partial class Form1 : Form
	{
		...
		private void UrunleriListele()
		{
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// *** CmbUrunFiltre artık doldurulmuyor ve kullanılmıyor ***
				...
				// CmbUrunFiltre.SelectedIndex = 0 kaldırıldı
		...
		private void HareketleriListele()
		{
			...
			// *** CmbUrunFiltre'ye yapılan başvurular kaldırıldı ***
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// int urunId = (CmbUrunFiltre.SelectedItem as ComboboxItem).Value; kaldırıldı

				// Tarih filtrelerini kullan (Bitiş tarihi olarak DateTime.Now kullanıldı)
				...
				// if (urunId != 0) sql += " AND h.UrunId = $urunId"; kaldırıldı

				sql += " ORDER BY h.TarihSaat DESC";

				// Sorguyu çalıştırıp tabloya bağlama
				...
				// cmd.Parameters.AddWithValue("$urunId", urunId); kaldırıldı

				DataTable dt = new DataTable();
				dt.Load(cmd.ExecuteReader());

				// dataGridView2'ye veriyi bağla
		...
		// Form1.cs içinde bu metodu bulun ve aşağıdaki ile değiştirin:
		...
		private void BtnHareketEkle_Click_1(object sender, EventArgs e)
		{
			...
			// Ürün Id'sinin 0 olup olmadığını kontrol et (Eğer ComboboxItem'in Value'su 0 ise)
			if (secilen.Value == 0)
			{
		...
		{
			// 1. Seçim kontrolü (dataGridView2'de satır seçili mi?)
			...
			string hareketTuru = dataGridView2.SelectedRows[0].Cells["HareketTuru"].Value.ToString();
			int hareketMiktari = Convert.ToInt32(dataGridView2.SelectedRows[0].Cells["Miktar"].Value);
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				try
				{
					// 1. Seçili hareketin UrunId'sini bul
					var cmdUrunId = connection.CreateCommand();
					cmdUrunId.Transaction = trans;
					cmdUrunId.CommandText = "SELECT UrunId FROM StokHareketleri WHERE Id = $hid;";
					cmdUrunId.Parameters.AddWithValue("$hid", hareketId);
					int urunId = Convert.ToInt32(cmdUrunId.ExecuteScalar());


					// 2. Stok tablosunda ters işlem yap (Stok Dengeleme)
					var cmdS = connection.CreateCommand();
					cmdS.Transaction = trans;

					if (hareketTuru == "GİRİŞ")
					{
						// Giriş silinirse stok düşer
						cmdS.CommandText = "UPDATE Urunler SET MevcutStok = MevcutStok - $m WHERE Id = $id;";
					}
					else // "ÇIKIŞ"
					{
						// Çıkış silinirse stok artar
						cmdS.CommandText = "UPDATE Urunler SET MevcutStok = MevcutStok + $m WHERE Id = $id;";
					}
					...
					// 3. Stok Hareketini sil
					var cmdH = connection.CreateCommand();
					cmdH.Transaction = trans;
					cmdH.CommandText = "DELETE FROM StokHareketleri WHERE Id = $hid";
					cmdH.Parameters.AddWithValue("$hid", hareketId);
					cmdH.ExecuteNonQuery();
			...
			// Silindikten sonra listeyi ve ürün stoklarını güncelle
			HareketleriListele();
			UrunleriListele();
		}
		...
		private void BtnGuncelle_Click(object sender, EventArgs e)
		{
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				var cmd = connection.CreateCommand();
				// Sorguya MevcutStok alanı dahil edildi.
				cmd.CommandText =
				@"UPDATE Urunler SET
					UrunAdi = $a,
					StokKodu = $s,
				...
				cmd.Parameters.AddWithValue("$s", TxtStok.Text);
				cmd.Parameters.AddWithValue("$b", birimFiyat);
				cmd.Parameters.AddWithValue("$m", mevcutStokGuncel); // Yeni Parametre
				cmd.Parameters.AddWithValue("$id", id);
		...
		// Ürün Filtresi (CmbUrunFiltre) kaldırıldığı için bu metot silinmiştir.
		/*
		...
		*/
		...
		{
			// Bu metot adının UI'da "Hareket Sil" butonu olduğunu varsayıyoruz.

			if (dataGridView2.SelectedRows.Count == 0)
			{
				MessageBox.Show("Silmek için hareket seç.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				try
				{
					...
					var cmdUrunId = connection.CreateCommand();
					cmdUrunId.Transaction = trans;
					cmdUrunId.CommandText = "SELECT UrunId FROM StokHareketleri WHERE Id=$id";
					cmdUrunId.Parameters.AddWithValue("$id", hareketId);
					int urunId = Convert.ToInt32(cmdUrunId.ExecuteScalar());
					...
					cmdSil.CommandText = "DELETE FROM StokHareketleri WHERE Id=$id";
					cmdSil.Parameters.AddWithValue("$id", hareketId);
			...
			// İşlem sonrası listeleri yenile
			HareketleriListele();
			UrunleriListele();
		}
		...
		private void CmbFiltre_SelectedIndexChanged_1(object sender, EventArgs e)
		{
			UrunFiltrele();
		}
	}
