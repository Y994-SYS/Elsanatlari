...
namespace Elsanatlari
{
	public partial class Form1 : Form
	{
		...
		{
			// *** NULL KONTROLÜ VE ARGUMENTOUTOFRANGE KONTROLÜ (GÜVENLİK) ***
			// ComboBox'ta hiç öğe yoksa (örneğin ürün yoksa) index ataması yapmamak için Count kontrolü.
			if (cmbHareketFiltre.SelectedItem == null && cmbHareketFiltre.Items.Count > 0)
			{
				cmbHareketFiltre.SelectedIndex = 0;
			}
			if (CmbUrunFiltre.SelectedItem == null && CmbUrunFiltre.Items.Count > 0)
			{
			...
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// Güvenli string ataması (null ise "Tümü" veya boş string atar, hata vermez)
				string hareketTuru = cmbHareketFiltre.SelectedItem?.ToString() ?? "Tümü";

				int urunId = 0;
				// Sadece öğe seçiliyse ID'yi al
				if (CmbUrunFiltre.SelectedItem is ComboboxItem)
				{
				...
				// *** SQL SYNTAX HATASI ÇÖZÜMÜ: ORDER BY'dan önce boşluk eklendi ***
