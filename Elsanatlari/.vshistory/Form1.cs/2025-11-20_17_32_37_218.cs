	...
	public partial class Form1 : Form
	{
		...
		private void HareketleriListele()
		{
			using (var connection = new SqliteConnection(connectionString))
			{
				...
				// Hata çözüldü: cmbHareketFiltre'deki değer string olduğu için direkt .ToString() kullanılır.
				string hareketTuru = cmbHareketFiltre.SelectedItem.ToString();

				// CmbUrunFiltre'deki değer ComboboxItem olduğu için Value özelliği alınır.
				int urunId = ((ComboboxItem)CmbUrunFiltre.SelectedItem).Value;

				// Tarih saat değerleri formatlanarak alınıyor.
